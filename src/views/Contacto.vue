<template>
  <div class="my-16">
    <div class="bg-gray-800 text-gray-100 py-8 px-0 sm:px-2 md:px-8">
      <div
        class=" my-4 max-w-screen-xl mt-24 px-8 grid gap-8 grid-cols-1 md:grid-cols-2 md:px-12 lg:px-16 xl:px-32 py-16 mx-auto bg-gray-100 text-gray-900 rounded-lg shadow-lg">
        <div class="flex flex-col justify-between text-center">
          <div class="px-6 text-center">
            <h2 class="text-4xl lg:text-6xl font-bold leading-tight text-center">¡Contáctate con nosotros!</h2>
            <div class="text-gray-700 mt-8 mb-4">
              ¿Quieres vender tu propiedad o quieres conocer nuestros servicios adicionales? <br>¡Déjanos un mensaje y te contactaremos cuanto antes!
            </div>
            <router-link to="/aviso-de-privacidad" class="font-semibold underline text-yellow-700">¿Cómo usamos tus datos? <i class="fas fa-arrow-right"></i></router-link>
          </div>
        ...
        <div class="px-4">
          <div>
            <span class="uppercase text-sm text-gray-600 font-bold">Tipo de solicitud</span>
            <select @change="selectOption($event)" name="state" autocomplete="state" class="w-full bg-gray-50 text-gray-900 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline shadow">
              <option value="none" selected disabled>Seleccione una opción</option>
              <option value="1">Quiero vender mi propiedad</option>
              <option value="2">Terrenos comerciales y otros servicios</option>
              <option value="3">Ya soy cliente y necesito ayuda</option>
            </select>
            <div v-if="infoVenta" class="hidden" >
              <span class="font-semibold text-indigo-600">Por el momento, solamente disponible para clientes en el área de Matías Romero, OAX</span>
            </div>
          </div>
          <div class="mt-4">
            <span class="uppercase text-sm text-gray-600 font-bold">Nombre</span>
            <input v-model="nombre" class="w-full bg-gray-50 text-gray-900 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline"
              type="text" placeholder="Tu nombre completo" autocomplete="given-name">
          </div>
          <div class="mt-4">
            <span class="uppercase text-sm text-gray-600 font-bold">Correo electrónico</span>
            <input v-model="email" class="w-full bg-gray-50 text-gray-900 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline"
              type="text"
              autocomplete="email"
          ...
          <div class="mt-4">
            <span class="uppercase text-sm text-gray-600 font-bold">Teléfono de contacto</span>
            <input v-model="tel" class="w-full bg-gray-50 text-gray-900 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline"
              type="text"
              autocomplete="phone"
          ...
          <div class="mt-4">
            <span class="uppercase text-sm text-gray-600 font-bold">Información adicional</span>
            <textarea
              v-model="mensaje"
              :placeholder="ph"
              class="w-full h-32 bg-gray-50 text-gray-900 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline"></textarea>
          </div>
          <div class="mt-4">
            <button
              @click="sendEmail"
              class="uppercase text-sm font-bold tracking-wide hover:bg-indigo-600 bg-indigo-500 text-gray-100 p-3 rounded-lg w-full focus:outline-none focus:shadow-outline transition-all ease-in-out duration-300">
              <i v-if="loading" class="fas fa-spinner self-center animate-spin mr-2"></i>
              <span v-if="loading" class="font-semibold">Un momento...</span>
              <span v-if="!loading" class="font-semibold">Enviar mensaje</span>
              <i v-if="!loading" class="fas fa-envelope pl-2"></i>
            </button>
          </div>
          <div class="hidden mt-4 sm:mt-24">
            <a v-if="Comercial"
              href="https://wa.link/9wfpgj"
              target="_blank"
              class="font-bold tracking-wide bg-green-500 hover:bg-green-700 text-gray-100 p-3 rounded-lg w-full focus:outline-none focus:shadow-outline transition-all ease-in-out duration-300 flex justify-center align-middle">
              <i v-if="loading" class="fas fa-spinner self-center animate-spin mr-2"></i>
              <span v-if="loading" class="font-semibold">Un momento...</span>
              <i v-if="!loading" class="fab fa-whatsapp text-xl text-gray-50 pr-2"></i>
              <span v-if="!loading" class="text-xl text-gray-50 font-semibold">Enviar mensaje</span>
            </a>
            <a v-if="Otro"
              href="https://wa.link/5hu6rv"
              target="_blank"
              class="font-bold tracking-wide bg-green-500 hover:bg-green-700 text-gray-100 p-3 rounded-lg w-full focus:outline-none focus:shadow-outline transition-all ease-in-out duration-300 flex justify-center align-middle">
...
export default {
  ...
  data () {
    return {
      ...
      email: '',
      tel: '',
      ph: 'Por favor, primero selecciona una opción'
    }
  },
  methods: {
    selectOption (e) {
      ...
      switch (this.opcion) {
        case '1':
          ...
          this.Otro = false
          this.ph = 'Si desea vender su propiedad, ingrese el domicilio aquí. De lo contrario, especifique un mensaje adicional'
          break
        case '2':
          this.tipo = 'Comercial'
          this.soloVenta = false
          this.Comercial = true
          this.Otro = false
          this.ph = '¿Qué tipo de terrenos estás buscando y en dónde?'
          break
        case '3':
          this.tipo = 'Otro'
          this.soloVenta = false
          this.Comercial = false
          this.Otro = true
          this.ph = 'Deja un mensaje y un asesor te contactará lo más pronto posible'
      }
    },
    sendEmail () {
      switch (this.tipo) {
        case 'Venta':
          if (
            ...
            this.$message.error('Por favor, complete todos sus datos e inténtelo de nuevo.')
          } else {
            this.loading = true
            emailjs.send('service_kh0cax6', 'template_9g9mf2l', {
              nombre: this.nombre,
              datos: this.mensaje,
              tel: this.tel,
              email: this.email
              ...
              this.$message({
                message: `¡Gracias por contactarnos! Se le enviará un correo de confirmación a ${this.email}`,
                type: 'success',
                offset: 35
              })
              this.nombre = null
        ...
        case 'Comercial':
          if (
            ...
            this.loading = true
            emailjs.send('service_kh0cax6', 'template_oe5ncvk', {
              nombre: this.nombre,
              datos: this.mensaje,
              tel: this.tel,
              email: this.email
        ...
        case 'Otro':
          if (
            ...
            this.loading = true
            emailjs.send('service_kh0cax6', 'template_3g5krc9', {
              nombre: this.nombre,
          ...
          break
        default:
          this.$message.error('Por favor, seleccione una opción primero.')
          break
      }
    }
  },
