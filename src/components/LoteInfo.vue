<template>
  <AgendaCita ref="Cita" Lote :nombreLote="lote.Nombre" :idCasa="idCasa" :Ubicacion="lote.DireccionCompleta" :tipo="tipoRuta" />
  <div class="mb-48">
      <div id="portada" :style="{backgroundImage: 'linear-gradient(180deg, rgba(231,231,231,0.09007352941176472) 0%, rgba(11,7,5,0.5942752100840336) 56%, rgba(0,0,0,0.8127626050420168) 98%), url(' + portada + ')', backgroundSize: 'cover', backgroundPosition: 'center', height: '30rem' }" class="relative z-10 shadow-xl">
        ...
        <div class="flex w-full justify-between absolute bottom-2 px-4 sm:px-8 md:px-16 z-50 text-white">
          <div class="text-left">
            <h2>Lote en fraccionamiento {{descripcion1}} </h2>
        ...
        <div class="flex flex-col-reverse sm:flex-row">
          <div class="w-full sm:w-2/3 mx-0 sm:ml-4 p-4">
            <div class="text-left">
              ...
              <div>
                <p class="lato text-left text-lg text-gray-700">{{lote.Descripcion}}</p>
              </div>
            </div>
          ...
          <div id="action" class="appearance-none side border-l p-4 sm:pl-4 border-gray-300 whitespace-normal">
            ...
            <div class="flex flex-col px-4 mt-4 mb-6 text-left justify-start">
              <div class="inline-flex mx-4">
                ...
                <span>{{lote.Superficie}} m<sup>2</sup></span>
              </div>
              <div class="inline-flex mx-4">
                <i class="text-green-700 w-5 fas fa-map-marker-alt mr-2 self-start mt-1"></i>
                <span class="font-semibold">{{lote.DireccionCompleta}}</span>
              </div>
            </div>
            <div class="mb-6">
              <p class="text-gray-700 monserrat text-base">Aprovecha la oportunidad de expandir tu patrimonio. <br>Contamos con planes de pago a tu medida<span><sup>*</sup></span>. ¡Descrubre lo sencillo que es!</p>
            </div>
            <button @click="$refs.Cita.openModal()" class="focus:outline-none hover:bg-yellow-700 active:bg-yellow-500 inline-flex justify-center text-lg w-full uppercase text-center font-semibold bg-yellow-600 text-white rounded-lg px-8 py-2 transition-all ease-in-out duration-300">
              <span class="mr-4">¡Quiero éste lote!</span>
...
export default {
  ...
  mounted () {
    ...
    this.idM = this.$route.params.idM
    this.idLote = this.$route.params.idL
    this.ruta = `Ubicaciones/${this.idEstado}/${this.idCiudad}/Fraccionamientos/${this.idFraccFrom}/manzanas/${this.idM}/lotes/${this.idLote}`
    ...
    firebase.firestore().collection('Ubicaciones').doc(this.idEstado).collection('Ciudades').doc(this.idCiudad).collection('Fraccionamientos').doc(this.idFraccFrom).collection('manzanas').doc(this.idM).get().then(mz => {
      this.manzanaDesde = mz.data().Nombre
    })
    firebase.firestore().collection('Ubicaciones').doc(this.idEstado).collection('Ciudades').doc(this.idCiudad).collection('Fraccionamientos').doc(this.idFraccFrom).collection('manzanas').doc(this.idM).collection('lotes').doc(this.idLote).get().then(lote => {
      this.lote = lote.data()
      this.lote.DireccionCompleta = 'Fraccionamiento ' + this.descripcion1 + ', ' + this.manzanaDesde + ', ' + this.descripcion2
      var formatter = new Intl.NumberFormat('en-US', {
        style: 'currency',
        currency: 'USD'
      })
      this.precioFormat = formatter.format(lote.data().Precio)
    }).then(() => {
      storage.child(`Ubicaciones/${this.idEstado}/${this.idCiudad}/Fraccionamientos/${this.idFraccFrom}/manzanas/${this.idM}/lotes/${this.idLote}/portada/portada.jpg`).getDownloadURL().then(url => {
        this.portada = url
        this.srcList.push(url)
      })
      storage.child(`Ubicaciones/${this.idEstado}/${this.idCiudad}/Fraccionamientos/${this.idFraccFrom}/manzanas/${this.idM}/lotes/${this.idLote}/galeria/foto_1.jpg`).getDownloadURL().then(url => {
        this.foto1 = url
        this.srcList.push(url)
      })
      storage.child(`Ubicaciones/${this.idEstado}/${this.idCiudad}/Fraccionamientos/${this.idFraccFrom}/manzanas/${this.idM}/lotes/${this.idLote}/galeria/foto_2.jpg`).getDownloadURL().then(url => {
        this.foto2 = url
        this.srcList.push(url)
      })
      storage.child(`Ubicaciones/${this.idEstado}/${this.idCiudad}/Fraccionamientos/${this.idFraccFrom}/manzanas/${this.idM}/lotes/${this.idLote}/galeria/foto_3.jpg`).getDownloadURL().then(url => {
        this.foto3 = url
        this.srcList.push(url)
      })
      storage.child(`Ubicaciones/${this.idEstado}/${this.idCiudad}/Fraccionamientos/${this.idFraccFrom}/manzanas/${this.idM}/lotes/${this.idLote}/galeria/foto_4.jpg`).getDownloadURL().then(url => {
        this.foto4 = url
        this.srcList.push(url)
      })
      storage.child(`Ubicaciones/${this.idEstado}/${this.idCiudad}/Fraccionamientos/${this.idFraccFrom}/manzanas/${this.idM}/lotes/${this.idLote}/galeria/foto_5.jpg`).getDownloadURL().then(url => {
        this.foto5 = url
        this.srcList.push(url)
      })
    })
